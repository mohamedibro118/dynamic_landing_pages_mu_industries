...
<div data-name="{{$name}}" data-backdrop-can-close="{{$backdrop_can_close}}" style="padding: 0px 20px"
    ...
    <div class="sm:{{$sizes[$size]}} w-full p-4 m-auto bw-{{$name}} animate__faster">
        <div class="bg-white dark:bg-slate-900 dark:border dark:border-slate-800 rounded-lg drop-shadow-2xl">
            <div class="{{(!empty($type))?'flex':'flex-initial'}}">
                @if(!empty($type))
                <div class="modal-icon py-6 p-6 grow-0">
                    <x-bladewind::modal-icon type="{{ $type }}"></x-bladewind::modal-icon>
                </div>
               @endif
                <div class="modal-body grow p-6" style="padding:0px 20px !important">
            ...
            @if( $show_action_buttons )
                <div class="modal-footer   bg-gray-100 dark:bg-slate-800/50 dark:border-t dark:border-slate-800 py-3 px-6 rounded-br-lg rounded-bl-lg" style="justify-content: center !important">
                    <x-bladewind::button
                        type="secondary"  
                        size="{{$button_size}}" 
