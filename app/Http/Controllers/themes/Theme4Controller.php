...
class Theme4Controller extends Controller
{
    ...
    {
        return view('dashbourd.themes.theme4.index');
    }
    ...
    public function create()
    {
        ...
        return view('dashbourd.themes.theme4.create', [
            'page' => $page,
    ...
    public function store(Request $request)
    {
        ...
        try {
            $themeTitle = "theme4"; // Assuming you have a way to determine the theme
                ...
                foreach ($request->file('logos') as $sectionTitle => $logoFiles) {
                    ...
                    if (is_array($logoFiles)) {
                        foreach ($logoFiles as $identifier => $logo) {
                            $logoPath = $this->uploadeImg($logo, 'theme4/logos',250,250); // Store the logo image
            ...
            if ($request->file('gallary')) {
                ...
                foreach ($request->file('gallary') as $sectionTitle => $gallaryFiles) {
                    ...
                    if (is_array($gallaryFiles)) {
                        ...
                        $data['gallary'] = json_encode($this->uplaodeGallary($gallaryFiles, 'theme4/gallary'));
                        Gallary::create($data);
                    }
                }
            }
                ...
                foreach ($request->file('backgrounds') as $sectionTitle => $bgFiles) {
                    ...
                    if (is_array($bgFiles)) {
                        foreach ($bgFiles as $identifier => $image) {
                            $imagePath = $this->uploadeImg($image, 'theme4/backgrounds',1920,1080); // Store the logo image
                ...
                foreach ($request->file('photos') as $sectionTitle => $phFiles) {
                    ...
                    if (is_array($phFiles)) {
                        foreach ($phFiles as $identifier => $image) {
                            $imagePath = $this->uploadeImg($image, 'theme4/photos',1000,750); // Store the logo image
                ...
                foreach ($request->input('unitData') as $sectionTitle => $data) {
                    ...
                    if (is_array($data)) {
                        foreach ($data as $unit) {
                            // $photoPath = $this->uploadeCard_Photo($unit['url_logo'], 'theme4/units/photos'); // Store the logo image
                            $unitop = new Unit();
                ...
                foreach ($request->input('featurData') as $sectionTitle => $data) {
                    ...
                    if (is_array($data)) {
                        foreach ($data as $feature) {
                            // $photoPath = $this->uploadeImg_Logo($feature['url_icon'], 'theme4/features'); // Store the logo image
    ...
    {
        return view('dashbourd.themes.theme4.s_template');
    }
    ...
    public function edit($id)
    {
        ...
        return view('dashbourd.themes.theme4.edit', [
            'pageId' => $id,
            'page' => $page,
            'sections' => $sections,
            'descriptions' => $descriptions,
    ...
    public function uploadeUnitPhoto(Request $request)
    {
        ...
        if ($request->file('unit_photo')) {
            $url_Path = $this->uploadeImg($request->file('unit_photo'), 'theme4/units',520,400);
        }
        return response()->json([
    ...
    public function uploadeFeatureIcon(Request $request)
    {
        ...
        if ($request->file('feature_icons')) {
            $url_Path = $this->uploadeImg($request->file('feature_icons'), 'theme4/features',250,250);
        }
        return response()->json([
            'url_Path' => asset($url_Path),
        ]);
