...
<x-bladewind.button tag="a" href="{{ route('dashbourd.landing_pages.index') }}" size="tiny">Back</x-bladewind.button>
    <form action="{{route('dashbourd.themes.theme5.update',['id'=>$pageId]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('updateButton').addEventListener('click', function() {
                ...
                if (closestForm) {
                    ...
                    fetch("{{ route('dashbourd.themes.theme5.update',['id'=>$pageId]) }}", {
                            method: "POST",
                            body: formData
                        })
