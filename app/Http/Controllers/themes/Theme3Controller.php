...
class Theme3Controller extends Controller
{
    ...
    {
        return view('dashbourd.themes.theme3.index');
    }
    ...
    public function create()
    {
        ...
        return view('dashbourd.themes.theme3.create', [
            'page' => $page,
    ...
    public function store(Request $request)
    {
        $request->validate([
            'page_title' => [
                ...
                'string',
                'max:255',
                Rule::unique('l_pages','title')->where(function ($query) {
                    return $query->where('agent_id', Auth::guard('admin')->user()?->agent_id);
                }),
            ],
        ]);
        DB::beginTransaction();
        ...
        try {
            $themeTitle = "theme3"; // Assuming you have a way to determine the theme
                ...
                foreach ($request->file('logos') as $sectionTitle => $logoFiles) {
                    ...
                    if (is_array($logoFiles)) {
                        foreach ($logoFiles as $identifier => $logo) {
                            $logoPath = $this->uploadeImg($logo, 'theme3/logos', 250, 250); // Store the logo image
            ...
            if ($request->file('gallary')) {
                ...
                foreach ($request->file('gallary') as $sectionTitle => $gallaryFiles) {
                    ...
                    if (is_array($gallaryFiles)) {
                        ...
                        $data['gallary'] = json_encode($this->uplaodeGallary($gallaryFiles, 'theme3/gallary'));
                        Gallary::create($data);
                    }
                }
            }
                ...
                foreach ($request->file('backgrounds') as $sectionTitle => $bgFiles) {
                    ...
                    if (is_array($bgFiles)) {
                        foreach ($bgFiles as $identifier => $image) {
                            $imagePath = $this->uploadeImg($image, 'theme3/backgrounds', 1920, 1080); // Store the logo image
                ...
                foreach ($request->file('photos') as $sectionTitle => $phFiles) {
                    ...
                    if (is_array($phFiles)) {
                        foreach ($phFiles as $identifier => $image) {
                            $imagePath = $this->uploadeImg($image, 'theme3/photos', 1000, 750); // Store the logo image
            ...
            if ($request->input('descriptions') && is_array($request->input('descriptions')) && $this->multiDimensionArrayNotEmpty($request->input('descriptions'))) {
             ...
             //add color
             if ($request->input('colors') && $this->multiDimensionArrayNotEmpty($request->input('colors'))) {
                foreach ($request->input('colors') as $sectionTitle => $mycolors) {
                    $section = Section::where('l_page_id', $page->id)->where('identifier', $sectionTitle)->first();
                    // Create a new logo entry for the specific theme and page
                    if ($this->multiDimensionArrayNotEmpty($mycolors)) {
                        foreach ($mycolors as $identifier => $color) {
                ...
                foreach ($request->input('unitData') as $sectionTitle => $data) {
                    ...
                    if (is_array($data)) {
                        foreach ($data as $unit) {
                            // $photoPath = $this->uploadeCard_Photo($unit['url_logo'], 'theme3/units/photos'); // Store the logo image
                            $unitop = new Unit();
                ...
                foreach ($request->input('featurData') as $sectionTitle => $data) {
                    ...
                    if (is_array($data)) {
                        foreach ($data as $feature) {
                            // $photoPath = $this->uploadeImg_Logo($feature['url_icon'], 'theme3/features'); // Store the logo image
                            $featureop = new PageFeature();
                            $featureop->section_id = $section->id;
                            $featureop->identifier = 'feature_' . $i;
    ...
    {
        return view('dashbourd.themes.theme3.s_template');
    }
    ...
    public function edit($id)
    {
        ...
        $section_detail_id = $sections->where('identifier', 'details')->first()?->id;
        $section_feature_id = $sections->where('identifier', 'features')->first()?->id;;
        ...
        $details = PageFeature::where('section_id', $section_detail_id)->get();
        $features = PageFeature::where('section_id', $section_feature_id)?->get();
        ...
        $unitTypes = UnitType::all();
        return view('dashbourd.themes.theme3.edit', [
            'pageId' => $id,
            'page' => $page,
            'sections' => $sections,
            'descriptions' => $descriptions,
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            'page_title' => [
                ...
                'string',
                'max:255',
                Rule::unique('l_pages','title')->where(function ($query) use ($id) {
                    return $query->where('agent_id', Auth::guard('admin')->user()?->agent_id)->where('id', '!=', $id);
                }),
            ],
        ]);
        DB::beginTransaction();
            ...
            if ($request->input('descriptions') && $this->multiDimensionArrayNotEmpty($request->input('descriptions'))) {
               ...
               if ($request->input('colors') && $this->multiDimensionArrayNotEmpty($request->input('colors'))) {
                foreach ($request->input('colors') as $sectionTitle => $mycolors) {
                    ...
                    if ($this->multiDimensionArrayNotEmpty($mycolors)) {
                        foreach ($mycolors as $identifier => $color) {
                           Color::query()->updateOrCreatecolor($section->id, $identifier, $color);
                        }
                    }
                }
            }
    ...
    public function uploadeUnitPhoto(Request $request)
    {
        ...
        if ($request->file('unit_photo')) {
            $url_Path = $this->uploadeImg($request->file('unit_photo'), 'theme3/units', 520, 400);
        }
        return response()->json([
    ...
    public function uploadeFeatureIcon(Request $request)
    {
        ...
        if ($request->file('feature_icons')) {
            $url_Path = $this->uploadeImg($request->file('feature_icons'), 'theme3/features', 520, 400);
        }
        return response()->json([
            'url_Path' => asset($url_Path),
        ]);
