...
class Theme1Controller extends Controller
{
    ...
    {
        return view('dashbourd.themes.theme1.index');
    }
    ...
    public function create()
    {
        ...
        return view('dashbourd.themes.theme1.create', [
            'page' => $page,
    ...
    {  
        ...
        try {
            $themeTitle = "theme1"; // Assuming you have a way to determine the theme
                ...
                foreach ($request->file('logos') as $sectionTitle => $logoFiles) {
                    ...
                    if (is_array($logoFiles)) {
                        foreach ($logoFiles as $identifier => $logo) {
                            $logoPath = $this->uploadeImg($logo, 'theme1/logos',250,250); // Store the logo image
                ...
                foreach ($request->file('backgrounds') as $sectionTitle => $bgFiles) {
                    ...
                    if (is_array($bgFiles)) {
                        foreach ($bgFiles as $identifier => $image) {
                            $imagePath = $this->uploadeImg($image, 'theme1/backgrounds',1920,1080); // Store the logo image
                ...
                foreach ($request->file('photos') as $sectionTitle => $phFiles) {
                    ...
                    if (is_array($phFiles)) {
                        foreach ($phFiles as $identifier => $image) {
                            $imagePath = $this->uploadeImg($image, 'theme1/photos',1000,750); // Store the logo image
                ...
                foreach ($request->input('unitData') as $sectionTitle => $data) {
                    ...
                    if (is_array($data)) {
                        foreach ($data as $unit) {
                            // $photoPath = $this->uploadeCard_Photo($unit['url_logo'], 'theme1/units/photos'); // Store the logo image
                            $unitop = new Unit();
                ...
                foreach ($request->input('featurData') as $sectionTitle => $data) {
                    ...
                    if (is_array($data)) {
                        foreach ($data as $feature) {
                            // $photoPath = $this->uploadeImg_Logo($feature['url_icon'], 'theme1/features'); // Store the logo image
    ...
    {
        return view('dashbourd.themes.theme1.s_template');
    }
    ...
    public function edit($id)
    {
        ...
        $photos = Photo::whereIn('section_id', $sectionIds)->get();
         $unitTypes = UnitType::all();
        return view('dashbourd.themes.theme1.edit', [
            'pageId' => $id,
            'page' => $page,
            'sections' => $sections,
            'descriptions' => $descriptions,
    ...
    public function uploadeUnitPhoto(Request $request)
    {
        ...
        if ($request->file('unit_photo')) {
            $url_Path = $this->uploadeImg($request->file('unit_photo'), 'theme1/units',520,400);
        }
        return response()->json([
    ...
    public function uploadeFeatureIcon(Request $request)
    {
        ...
        if ($request->file('feature_icons')) {
            $url_Path = $this->uploadeImg($request->file('feature_icons'), 'theme1/features',250,250);
        }
        return response()->json([
            'url_Path' => asset($url_Path),
        ]);
